...
public interface LocationRepository extends JpaRepository<Location, String> {
    ...
    @Query(value = "select id, sido_nm, sgg_nm, emd_nm, st_aswkt(loc_coords) as loc_coords, st_aswkt(cent_coords) as cent_coords, create_datetime, modify_datetime " +
            ...
            "order by st_distance_sphere(st_geomfromtext(?1), st_centroid(loc_coords)), sido_nm, sgg_nm, emd_nm", nativeQuery = true)
    List<Location> findByInDistance(String point, int meter);

    @Query(value = "select id, sido_nm, sgg_nm, emd_nm, st_aswkt(loc_coords) as loc_coords, st_aswkt(cent_coords) as cent_coords, create_datetime, modify_datetime " +
            "from location " +
            "where st_contains(loc_coords, st_geomfromtext(?1))", nativeQuery = true)
    Optional<Location> findContainLocation(String point);
}